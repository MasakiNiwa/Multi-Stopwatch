...
「＋ セット」で親を作り、展開した行の「＋ 子を追加」で内訳を作ります。例：資格勉強 → テキスト／問題演習／復習。
同じセット内で子を開始すると前の子は停止します。別セットや単独計測との同時進行は可能です。
親の丸ボタンは、計測中なら停止、停止中なら最後に使った子を再開します。初回は子を選んでください。
所属変更は子の編集画面から。子のグループは親に従います。セット削除は子を残して単独化し、「子ごとすべて削除」は詳細画面から確認付きで行えます。
全体合計・個別ランキングは子と単独計測だけを数え、セットは別の小計として表示します。

削除の取り消しは直前の1操作のみです。計測中の項目を削除して取り消すと、削除していた間も含めて計測を継続します。長期の保護にはJSONバックアップを利用してください。
